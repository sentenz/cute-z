                   ...
                   tiny_dnn::vec_t &data)
{
    ...
    //resized.save("/etc/sentenz/config/neuron/mnist/number_resized.bmp");

    // mnist dataset is "white on black", so negate required
    // black on white
    //    std::transform(resized.begin(),
    //                   resized.end(),
    ...
    std::transform(resized.begin(),
                   resized.end(),
                   std::back_inserter(data),
                   [=](uint8_t c) { return (255 - c) * (minv - maxv) / 255.0 + maxv; }
    );
}

int recognize(const std::string &dictionary, const std::string &src_filename)
{
