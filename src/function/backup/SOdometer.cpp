...
double SOdometer::getRawValue()
{
    ...
    #else
    static int count = 0;
    return .5 + .3*sin(2*M_PI*1000*count++/10000) + .25 * (double)qrand()/RAND_MAX;
    #endif
}
...
bool SOdometer::getHilbertValue(double &angle)
{
    if (this->isStatus())
    {
        #ifdef S_DEBUG_02
        ...
        if (fabs(this->value) > threshold)
        {
            ...
            if (this->prev)
            {
                angle = this->hilbert->angle(this->vphase[this->vphase.size()/2] - this->prev) * (this->diameter / this->resolution);

                // DEBUG
                //qDebug() << vphase[vphase.size()/2] << "-" << prev << "=" << vphase[vphase.size()/2] - prev << "\n";
            }
