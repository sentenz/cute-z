...
void SNFCController::set_property()
{
...
void SNFCController::setKeyA(const bool key)
{
    this->bUseKeyA = key;
}

bool SNFCController::is_first_block(uint32_t uiBlock)
{
...
bool SNFCController::is_trailer_block(uint32_t uiBlock)
{
    /// Test if we are in the small or big sectors
    if (uiBlock < 128)
    {
...
uint32_t SNFCController::get_trailer_block(uint32_t uiFirstBlock)
{
    /// Test if we are in the small or big sectors
...
bool SNFCController::unlock_card(void)
{
    if (this->magic2)
    {
...
int SNFCController::get_rats(void)
{
    int res;
...
bool SNFCController::transmit_bits(const uint8_t *pbtTx, const size_t szTxBits)
{
...
bool SNFCController::transmit_bytes(const uint8_t *pbtTx, const size_t szTx)
{
    int res;
...
bool SNFCController::authenticate(uint32_t uiBlock)
{
    mifare_cmd mc;
...
bool SNFCController::read_block(const uint32_t block, uint8_t abtData[])
{
...
// ToDo
bool SNFCController::read_card()
{
    return false;
}

bool SNFCController::write_block(const uint32_t block, uint8_t abtData[])
{
    /// Try to authenticate for the current sector
    if (!this->authenticate(this->get_trailer_block(block)))
    {
...
// ToDo
bool SNFCController::write_card()
{
    return false;
}
...
bool SNFCController::transmitBlock(const Action action, const uint32_t block, uint8_t data[])
{
    // Time
    //clock_t t1, t2;
    //t1 = clock();
...
bool SNFCController::transmitCard(const Action action)
{
    switch (action)
    {
        case Action::Read:
