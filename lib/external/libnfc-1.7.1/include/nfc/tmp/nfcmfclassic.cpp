...
NfcMfClassic::NfcMfClassic(bool keyA) : bUseKeyA(keyA)
{
    ...
    this->pnd = nfc_open(this->context, nullptr);
    if (this->pnd == nullptr)
    {
...
bool NfcMfClassic::transmitBlock(const Action action, const uint32_t block, uint8_t data[])
{
    ...
    switch (action)
    {
        case Action::Read:
        {
            ...
            if (nfc_initiator_select_passive_target(this->pnd, nmMifare, nullptr, 0, &nt) > 0)
            {
                result = this->read_block(block, data);
        ...
        case Action::Write:
        {
            ...
            if (nfc_initiator_select_passive_target(this->pnd, nmMifare, nullptr, 0, &nt) > 0)
            {
                result = this->write_block(block, data);
            }
        }
            break;
        default:
