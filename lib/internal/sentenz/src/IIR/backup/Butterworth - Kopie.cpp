...
auto &Butterworth::numCofHP()
{
    ...
    tmpNum = this->numCofLP();

    for (size_t i = 0; i < tmpNum.size(); ++i)
    {
        if (i % 2)
        {
            tmpNum.at(i) = -tmpNum.at(i);
        }
    }
...
void Butterworth::setMode()
{
    if (den.empty() || num.empty())
    {
        try
        {
            ...
            {
                throw std::runtime_error("Unknown filter mode in Butterworth::filter()\n");
            }
        }
        catch (const std::exception& e)
        {
            std::cerr << "exception caught: " << e.what() << "error: Butterworth '" << mode << "' try .at('low' | 'high')" << std::endl;
        }
        catch(...)
        {
...
double &Butterworth::getCoeff()
{
    return fcf;
}

void Butterworth::setCoeff(double coeff)
{
    if (!den.empty() || !num.empty())
    {
        den.clear();
