#ifndef SENCODER25PRSBE_H
#define SENCODER25PRSBE_H

#include <iostream>

#include <sys/time.h>
#include <stdio.h>
#include <stdint.h>
#include <stdbool.h>
#include <string.h>
#include <stdlib.h>
#include <sys/ioctl.h>
#include <linux/spi/spidev.h>


class SEncoder25PRSBE
{
public:
    SEncoder25PRSBE();
    ~SEncoder25PRSBE();
    const SEncoder25PRSBE& operator=(const SEncoder25PRSBE&) = delete;

    /// Member functions
    bool init();
    int get(double *value);

private:
    /// Variable members
    uint8_t computeCRC(uint8_t *byte, size_t size);

    /// Variable members
    bool one = false;
    uint8_t error_counter = 0;
    uint8_t error = 0;
    uint8_t gain = 0;
    uint8_t counter = 0;
    uint8_t crc = 0;
    double last = 0.0;
    double diff = 0.0;
    double phase = 0.0;
    unsigned int alpha = 0;
    const double factor = 360.0 / 16383.0;

    /// SPI members
    int fd;
    char *port = "/dev/spidev0.0";
    uint8_t tx[8];
    uint8_t rx[8];
    uint8_t mode = SPI_CPHA;
    uint8_t bits = 8;
    uint16_t delay = 0;
    uint32_t speed = 500000;
    struct spi_ioc_transfer tr;
    uint8_t crcArray[256] = {
    
    };
};

#endif // SENCODER25PRSBE_H
